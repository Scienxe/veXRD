...
alchemy.sg = ( function( sg ) {
  ...
  var makePlotPanel = function() {
    ...
    var plotElems = plots.map( function( plot, idx ) {
      ...
      left.addEventListener( 'click', ( function( z, e ) {
        return function() {
          plots[ z ].offset--;                    console.log( plots[ z ].offset );
          redraw();
        };
      }( idx ) ) );
      left.innerHTML = '&#9664;';
  ...
  var makePlot = function() {
    ...
    for( var i = 0; i < plots.length; i++ ) {
      ...
      plot.xMin = plot.offsetxVals[ 0 ];
      plot.xMax = plot.offsetxVals[ plot.offsetxVals.length - 1 ];
      setAxisBounds();
    }
    ...
    for( var i = 0; i < plots.length; i++ ) {
      ...
      var spectraStr = plot.offsetxVals.map( function ( elem, idx ) {
        var pt = new Point( parseFloat( elem - axes.xMin ), parseInt( normalize * plot.yVals[ idx ] ) );
