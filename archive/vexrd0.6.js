...
alchemy.vexrd = ( function( vexrd ) {
  ...
  var typeFace = '"OpenSans", Helvetica, sans-serif',
  ...
  */
  ...
  var makePlot = function() {
    ...
    for( var i = 0; i < plots.length; i++ ) {
      ...
      if( prefs.displayLegend ) {
        ...
        plotTitle.setAttribute( 'y', topMargin + prefs.fontSize + ( ( plots.length - 1 ) * filmStripSize * 0.8 ) - ( filmStripSize * i  * 0.8 ) );
        ...
        var sparkLineStr = [];
        if( prefs.plotAxis === "semiLog" ) {
          var sparkNorm = filmStripSize * 0.5 / ( Math.log( plot.yMax ) / Math.log( 10 ) );
        } else {
          var sparkNorm = filmStripSize * 0.5 / plot.yMax;
        }
        for( var j = 0; j < sparkLineSize; j++ ) {
          var ptX = ( yAxisMargin + plotWidth - ( sparkLineSize / 2 ) ) + 0.5 * j;
          var ptY = Math.round( 1000 * ( topMargin + prefs.fontSize + ( ( plots.length - 1 ) * filmStripSize * 0.8 ) - ( filmStripSize * i  * 0.8 ) - ( sparkNorm * yValsAdj[ sparkLineStart + j ] ) ) ) / 1000;
    ...
    filmStrips.forEach( function( icddNum, idx ) {
      ...
      strip.forEach( function( peak, stickIdx ) {
        if( peak.theta > axes.xMin && peak.theta < axes.xMax ) {
          ...
          if( ( filmStripSize * intensities[ stickIdx ] / strongest ) < 1 ) {
            var stickPx = 1;
          } else {
            var stickPx = filmStripSize * intensities[ stickIdx ] / strongest;
          }
          var stick = makeLine ( pt.x,
                                 pt.y + filmStripSize * ( 1 + idx ),
                                 pt.x,
                                 pt.y + filmStripSize * ( 1 + idx ) - stickPx
          );
  ...
  var drawAxes = function() {
    ...
    if( prefs.displayYaxis ) {
      ...
      yLabel.setAttributeNS( null, 'y', topMargin + ( plotHeight - filmStrips.length * filmStripSize ) / 2 );
      ...
      var transformStr = 'rotate( -90 ' + prefs.fontSize * 0.7 + ',' + ( topMargin + ( plotHeight - filmStrips.length * filmStripSize ) / 2 ) + ')';
  ...
  var msgOut = function( msg ) {
    if( messageDiv.firstChild ) {
      messageDiv.firstChild.style.color = '#bbb';
    }
